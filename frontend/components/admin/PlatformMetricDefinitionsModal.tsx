...
import { usePlatformMetricDefinitions, useCreatePlatformMetricDefinition, useDeletePlatformMetricDefinition } from '@/lib/hooks';
...
export function PlatformMetricDefinitionsModal({ onClose }: { onClose: () => void }) {
  const { t } = useTranslation();
  const { data, isLoading } = usePlatformMetricDefinitions();
  const createDefinition = useCreatePlatformMetricDefinition();
  const deleteDefinition = useDeletePlatformMetricDefinition();
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="font-display text-lg font-bold text-text">{t('admin.platformMetricDefTitle')}</h2>
          </div>
        ...
        <p className="text-xs leading-relaxed text-text-muted">{t('admin.platformMetricDefHint')}</p>
